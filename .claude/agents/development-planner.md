...
다음 유형의 작업은 **반드시 Playwright MCP 테스트를 통과해야만 완료 처리 가능**:

- Notion API 연동 구현
- NextAuth.js 인증 플로우 구현
- 공유 링크 생성/조회 API 구현
- PDF 다운로드 기능 구현
- 기타 외부 API 연동 및 핵심 비즈니스 로직 구현

**테스트 실행 절차 (생략 불가)**:

1. 구현 완료 후 Playwright MCP로 테스트 실행
2. 정상 케이스(Happy Path) + 에러 케이스 모두 검증
3. 테스트 미통과 시 → 구현 재검토 → 수정 → 재테스트 (루프 반복)
4. **모든 테스트 통과 확인 후에만 다음 단계 진행 허용**
...
- **🚨 API 연동 및 비즈니스 로직 구현 시 아래 테스트 절차 필수 이행**:
  1. Playwright MCP로 정상 흐름(Happy Path) 테스트 실행
  2. Playwright MCP로 에러 흐름(Error Flow) 테스트 실행
  3. Playwright MCP로 엣지 케이스(Edge Case) 테스트 실행
  4. **3가지 모두 통과 시에만 해당 단계 완료로 인정**
  5. 테스트 실패 → 구현 수정 → 재테스트 루프 반복 (건너뛰기 불가)
- **테스트를 수행하지 않으면 Task를 완료 처리할 수 없음**
...
#### **🧪 테스트 검증 (필수 — 미충족 시 완료 불가)**

> **원칙**: 아래 유형의 Task는 테스트 없이 완료 처리할 수 없습니다.

**테스트 필수 작업 유형:**

- [ ] **Notion API 연동** Task에 Playwright MCP 테스트 시나리오가 정의되었는가?
- [ ] **NextAuth.js 인증 플로우** Task에 Playwright MCP 테스트 시나리오가 정의되었는가?
- [ ] **공유 링크 생성/조회** Task에 Playwright MCP 테스트 시나리오가 정의되었는가?
- [ ] **PDF 다운로드** Task에 Playwright MCP 테스트 시나리오가 정의되었는가?

**테스트 기준:**

- [ ] 각 테스트 필수 Task의 작업 파일에 "## 테스트 체크리스트" 섹션이 포함되었는가?
- [ ] 테스트 체크리스트에 정상 흐름(Happy Path) 시나리오가 명시되었는가?
- [ ] 테스트 체크리스트에 에러 흐름(Error Flow) 시나리오가 명시되었는가?
- [ ] 테스트 체크리스트에 엣지 케이스(Edge Case) 시나리오가 명시되었는가?
- [ ] 정상 케이스와 에러 케이스 **모두** 통과해야 완료로 인정됨이 명시되었는가?
- [ ] Phase 3에 전체 플로우 통합 테스트 Task가 포함되었는가?

**🚫 테스트 없이 완료 처리 금지**: 위 유형에 해당하는 Task는 Playwright MCP 테스트 통과 없이 ✅ 완료 표시 불가
